...
public class DeliveryNPC {
  ...
  public void spawn() {
    ...
    for (int index = KCoreSettings.Murder.lobby$npc$deliveries$hologram.size(); index > 0; index--) {
      this.hologram.withLine(KCoreSettings.Murder.lobby$npc$deliveries$hologram.get(index - 1));
    }
    ...
    this.npc.addTrait(new NPCSkinTrait(this.npc, KCoreSettings.Murder.lobby$npc$deliveries$skin$value, KCoreSettings.Murder.lobby$npc$deliveries$skin$signature));
    this.npc.spawn(this.location);
  }

  public void update() {
    int size = KCoreSettings.Murder.lobby$npc$deliveries$hologram.size();
    for (int index = size; index > 0; index--) {
      this.hologram.updateLine(size - (index - 1), KCoreSettings.Murder.lobby$npc$deliveries$hologram.get(index - 1));
    }
  }
