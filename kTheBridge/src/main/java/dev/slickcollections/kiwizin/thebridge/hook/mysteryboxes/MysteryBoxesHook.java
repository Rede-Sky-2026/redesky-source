...
public class MysteryBoxesHook {
  ...
  public static void sync(CommandSender sender) {
    ...
    Database.getInstance().update("DELETE FROM `kMysteryBoxContent` WHERE `dependency` = ?", "kTheBridge");
  ...
  public static boolean isUnsynced() {
    if (unsynced == null) {
      if (Main.kMysteryBox && Database.getInstance().isSql()) {
        ...
        try (CachedRowSet rs = Database.getInstance().query("SELECT * FROM `kMysteryBoxContent` WHERE `dependency` = ?", "kTheBridge")) {
          if (rs != null) {
            rs.beforeFirst();
            while (rs.next()) {
