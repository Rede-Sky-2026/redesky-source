...
import dev.slickcollections.kiwizin.thebridge.lobby.DeliveryNPC;
...
public class NPCDeliveryCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§6/tb npcentregas adicionar [id] §f- §7Adicionar NPC.\n§6/tb npcentregas remover [id] §f- §7Remover NPC.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /tb npcentregas adicionar [id]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /tb npcentregas remover [id]");
        return;
      }
