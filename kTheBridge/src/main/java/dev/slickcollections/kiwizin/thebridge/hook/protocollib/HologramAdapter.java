package dev.slickcollections.kiwizin.thebridge.hook.protocollib;
    ...
    if (packet.getType() == PacketType.Play.Server.ENTITY_METADATA) {
      ...
      if (profile != null && name.contains("{deliveries}")) {
        ...
        if (deliveries == 0) {
          ...
          name = KCoreSettings.TheBridge.lobby$npc$deliveries$deliveries.replace("{deliveries}", StringUtils.formatNumber(deliveries)).replace("{s}", deliveries > 1 ? "s" : "");
        }
      }
